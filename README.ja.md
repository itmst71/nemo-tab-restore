...
`Ctrl+W` で閉じたタブを `Ctrl+Shift+T` で復元できるようになります。
...
現在タブの URI を履歴に保存したあと、通常どおり Nemo がタブを閉じます。

マウス操作で閉じたタブは履歴に保存されません。詳しくは [既知の制限](#既知の制限) を参照してください。
...
- キーボードによる閉じるショートカットのみ履歴に保存します。
- マウスによるタブを閉じる操作は履歴に保存されません。
- 対象外の操作:
  - タブの閉じるボタン
  - タブのミドルクリック
