...
Tumbleweed では現在、`python3-nemo` が対応する Python GObject パッケージを依存として引きます。
...
- close shortcut は現在の URI を保存したあと、Nemo の標準 close 処理に渡します。
- restore shortcut は履歴から URI を取り出し、既存の Nemo ウィンドウに開きます。
